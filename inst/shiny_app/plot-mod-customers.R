...
customersUI <- function(id) {
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      "Description",
      # Start fluid row
      includeHTML("descriptions/customers.html")
    ),
...
customers <- function(input, output, session, dataFilters, sharedInputs) {

  # Pause reactive --------------------------------------------------------
  observe({
    if ("customers" %in% sharedInputs$activePanel()) {
  ...
  # Show/hide fill input --------------------------------------------------
  observe({
    shinyjs::toggle("fill", input$yvar == "items")
  })
  ...
  queryTimer <- reactiveValues(start = NULL, stop = NULL, label = "Customers")
  ...
  summaryData <- pauseableReactive({
    ...
    future({
      ...
      if (yvar == "customers") {
        countData <- countCustomers(permitData, groupVarsSQL)
      } else if (yvar == "items") {
        countData <- countItems(permitData, groupVarsSQL)
      }
    }) %>% catch(function(reason) {
      showModal(genericError)
      removeNotification(req(customersMsg))
    })
  })
  ...
  callModule(savePlots, "spNS", gg, defaultFilename = "huntfishapp_customers")
  
  # Call save data module -------------------------------------------------
  callModule(saveData, "saveData", summaryData, defaultFilename = "huntfishapp_customers")
